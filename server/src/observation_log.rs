...
//!
//! The design is deliberately boring: configuration from the environment,
//! synchronous append, and — crucially — I/O errors are logged and swallowed,
//! never propagated, so logging can never break the solve loop.
...
// /home/cedar/run/cedar_observation_log.jsonl.
...
// Formats a SystemTime as ISO-8601 in the local timezone, to whole seconds:
// `%Y-%m-%dT%H:%M:%S%z`.
