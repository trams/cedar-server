...
use canonical_error::{
    failed_precondition_error, invalid_argument_error, CanonicalError,
};
use cedar_detect::histogram_funcs::{
    average_top_values, get_level_for_fraction, remove_stars_from_histogram,
};
use cedar_elements::{
    astro_util::{
...
struct SolveState {
    ...
    // In image coordinates (coordinate space of CapturedImage.image).
...
impl SolveEngine {
    ...
    pub async fn set_fov_estimate(
        ...
        if fov_estimate.is_some() && fov_estimate.unwrap() <= 0.0 {
            return Err(invalid_argument_error(
                format!(
                    "fov_estimate must be positive; got {}",
                    fov_estimate.unwrap()
                )
                .as_str(),
            ));
    ...
    // Sets the boresight pixel. `boresight_pixel` must be in image coordinates
    ...
    pub async fn set_distortion(
        ...
        if distortion < -0.2 || distortion > 0.2 {
            return Err(invalid_argument_error(
                format!(
                    "distortion must be in [-0.2, 0.2]; got {}",
                    distortion
                )
                .as_str(),
            ));
    ...
    pub async fn set_match_max_error(
        &mut self,
        match_max_error: f64,
    ) -> Result<(), CanonicalError> {
        if match_max_error < 0.0 {
            return Err(invalid_argument_error(
                format!(
                    "match_max_error must be non-negative; got {}",
                    match_max_error
                )
                .as_str(),
            ));
    ...
    async fn attempt_plate_solve(
        ...
        if have_stars || imu_estimate.is_some() {
            if !skip_stars {
                ...
                {
                    let interval =
                        now.duration_since(last_attempt).as_secs_f64();
    ...
    async fn handle_slew(
        ...
        let target_image_coord = ImageCoord {
            x: img_coord.x,
            y: img_coord.y,
        };
        ...
        let mut boresight_image_region = Some(
            Rect::at(
        ...
        let mut boresight_image = Some(
            image
                .view(
                    boresight_image_region.unwrap().left() as u32,
                    boresight_image_region.unwrap().top() as u32,
                    boresight_image_region.unwrap().width(),
                    boresight_image_region.unwrap().height(),
                )
                .to_image(),
        );
