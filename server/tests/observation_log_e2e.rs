...
//!
//! The solver here is a trivial stub returning a canned solved solution, so the
//! test needs neither the Python tetra3 subprocess nor a pattern database. It
//! exercises the wiring that matters: capture -> detect -> solve ->
//! `finalize_and_post_result` -> `maybe_log_observation` -> JSON Lines file.
//!
//! Unlike the plate-solve harness in `e2e_plate_solve.rs` this is not
//! `#[ignore]`d: it needs no staged corpus and no venv, only the demo image in
//! the repo, and runs in a couple of seconds as part of a normal `cargo test`.
...
// Mirror of `MyCedar::compute_binning` in cedar_server.rs, so the demo image is
// detected at the binning production would pick for a sensor of its size. (The
// plate-solve harness in common/harness.rs deliberately leaves binning at 1; its
// corpus is synthetic and small.) Production also returns a display_sampling
// flag above 12 Mpix, which does not affect detection, so the top two size
// buckets collapse to one here.
